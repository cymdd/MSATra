...
class GRUDecoder(nn.Module):
    ...
    def __init__(self, args) -> None:
        ...
        self.args = args
        self.input_size = self.args.hidden_size * 2
        self.hidden_size = self.args.hidden_size * 2
        self.future_steps = args.pred_length
        self.num_modes = args.final_mode
        self.min_scale = min_scale
        self.args = args
        self.extended_dim = nn.Linear(self.args.hidden_size, self.hidden_size)
    ...
    def forward(self, global_embed: torch.Tensor, hidden_state, cn) -> Tuple[torch.Tensor, torch.Tensor]:
        global_embed = self.extended_dim(global_embed)
        cn = self.extended_dim(cn)
